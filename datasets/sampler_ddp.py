...
def _serialize_to_tensor(data, group):
    ...
    # storage = torch.ByteStorage.from_buffer(buffer) # deprecated
    # tensor = torch.ByteTensor(storage).to(device=device)
    
    np_buffer = np.frombuffer(buffer, dtype=np.uint8).copy()  # <-- Make it writable
    tensor = torch.from_numpy(np_buffer).to(device=device)
